...
export default function PasswordChange() {
  ...
  const handleChange = (e) => {
    ...
    if (name === "newPassword") {
      ...
      if (!passwordPattern.test(cleanedValue)) {
        setErrors(prev => ({
          ...prev,
          newPassword: "비밀번호는 8~20자의 영문, 숫자, 특수문자(~!@#$%^&*)를 모두 포함해야 합니다.",
        }));
      } else {
        setErrors(prev => ({ ...prev, newPassword: "" }));
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.put(`${API_ENDPOINTS.USER}/password`, {
      ...
      setMessage(err.response?.data || "비밀번호 변경에 실패했습니다");
      setMessageType("error");
    } finally {
      setIsLoading(false);
    }
      ...
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        <div className={styles.formGroup}>
          ...
          <div className={styles.inputWrapper}>
            <img src={PasswordIcon} alt="현재 비밀번호" className={styles.inputIcon} />
            <input
          ...
          {focusedField === "currentPassword" && errors.currentPassword && form.currentPassword && (
        ...
        <div className={styles.formGroup}>
          ...
          <div className={styles.inputWrapper}>
            <img src={PasswordIcon} alt="새 비밀번호" className={styles.inputIcon} />
            <input
          ...
          {focusedField === "newPassword" && errors.newPassword && form.newPassword && (
            <div className={styles.errorText}>{errors.newPassword}</div>
          )}
          <div className={styles.helpText}>
            8~20자의 영문, 숫자, 특수문자(~!@#$%^&*)를 모두 포함해야 합니다
          </div>
        </div>
        ...
        <div className={styles.formGroup}>
          ...
          <div className={styles.inputWrapper}>
            <img src={PasswordIcon} alt="새 비밀번호 확인" className={styles.inputIcon} />
            <input
            ...
            />
            <img
              src={showPasswords.confirmNewPassword ? VisibleIcon : VisibleOffIcon}
              alt="비밀번호 보기"
              className={styles.toggleIcon}
              onClick={() => togglePasswordVisibility('confirmNewPassword')}
            />
          </div>
          {focusedField === "confirmNewPassword" && errors.confirmNewPassword && form.confirmNewPassword && (
            <div className={styles.errorText}>{errors.confirmNewPassword}</div>
          )}
        </div>
