...
export function joinSock(isOpen, chatId, senderId) {
  ...
  // STOMP 해제
  const disconnectStompClient = disconnect;
  ...
  useEffect(() => {
    ...
    console.log(`[joinSock] 채팅방 초기화 시작: ${chatId} (${effectiveSenderId})`); // effectiveSenderId
    ...
    const initializeChatRoom = async () => {
      ...
      if (restApiJoinSuccess) {
        ...
        try {
          ...
          setError(new Error("과거 메시지 조회 실패: " + (err.message || "알 수 없는 에러")));
        }
        // 그 후 STOMP 활성화
        activateStompClient();
      } else {
    ...
    initializeChatRoom(); // 정의된 로직을 여기서 호출합니다.
  ...
  return { messages, sendMessage, isLoading, error, isJoining, stompClient, disconnectStompClient };
