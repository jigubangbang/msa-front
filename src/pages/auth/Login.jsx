...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // 메시지를 백에서 주든 안 주든 일괄 처리
      const status = error.response?.status;
      const errorMsg = error.response?.data?.message;

      if (errorMsg) {
        setMessage(errorMsg); // 정지,탈퇴 메시지
      } else if (status === 401) {
        setMessage("인증에 실패했습니다. 다시 시도해 주세요.");
      } else if (status === 403) {
        setMessage("접근이 거부되었습니다. 관리자에게 문의하세요.");
      } else {
  ...
  const handleSocialLogin = (provider) => {
    ...
    if (provider === "kakao") {
      ...
      const REDIRECT_URI = "http://k8s-default-msafront-0961d08170-429312898.ap-northeast-2.elb.amazonaws.com/oauth/kakao/callback"; // 카카오에 등록한 Redirect URI
      const kakaoAuthURL = `https://kauth.kakao.com/oauth/authorize?client_id=${KAKAO_CLIENT_ID}&redirect_uri=${REDIRECT_URI}&response_type=code`;
      window.location.href = kakaoAuthURL;
    } else if (provider === "naver") {
      const NAVER_CLIENT_ID = import.meta.env.VITE_NAVER_CLIENT_ID;
      const REDIRECT_URI = "http://k8s-default-msafront-0961d08170-429312898.ap-northeast-2.elb.amazonaws.com/oauth/naver/callback";
      ...
      const REDIRECT_URI = "http://k8s-default-msafront-0961d08170-429312898.ap-northeast-2.elb.amazonaws.com/oauth/google/callback";
      const STATE = crypto.randomUUID();
      const SCOPE = "openid email profile";
      const googleAuthURL = `https://accounts.google.com/o/oauth2/v2/auth?response_type=code&client_id=${GOOGLE_CLIENT_ID}&redirect_uri=${REDIRECT_URI}&scope=${encodeURIComponent(
        SCOPE
